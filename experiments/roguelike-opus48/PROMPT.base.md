...
2. Test it: `node run_policy.js` — plays the **whole game** (no time limit) on seed 1 and prints
   per-game and aggregate metrics. Read them.
...
phases; the game can end. Your `policy()` must never throw and must always return a valid action.
...
## Seeds
By default you play seed 1. You may choose to practise on more seeds (`run_policy.js --seeds 1,2,3`) —
your call; focusing on one is fine. Your final policy will also be checked on a **separate, held-out
set of seeds you do not see**, so prefer an approach that generalizes rather than fitting one seed.

## Deliverables (leave both in this directory)
- `policy.js` — your best policy.
