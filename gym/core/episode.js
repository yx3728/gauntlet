 ...
 * written against:
 *   - the policy (and its optional `init()`) is called in a try/catch: a
 *     throwing policy ends the episode with `policy_error` recorded — it never
 *     crashes the batch;
 ...
 * @returns {object} result  `{ ...metrics, seed, steps, events, policy_error?, _gamelog? }`
 *                           (harness fields win over same-named task metrics)
 */
function runEpisode(taskMod, policyMod, seed, config = {}, opts = {}) {
  ...
  let mem = {};
  let policy_error;
  if (typeof policyMod.init === "function") {
    try {
      mem = policyMod.init();
    } catch (e) {
      policy_error = String((e && e.message) || e); // contained like a throwing policy(): steps stays 0
    }
  }
  ...
  while (!policy_error && steps < cap && !last.done) {
    let out;
    try {
  ...
  // Harness fields take precedence: obs.metrics keys are reserved per CONTRACT.md §3
  // (seed/steps/events/policy_error/_gamelog) and must never clobber the envelope.
  const metrics = (last.obs && last.obs.metrics) || {};
  const result = Object.assign({}, metrics, { seed, steps, events });
